...
export class CurrentStyling {
  ...
  public setStylesFromNodeHeirarchy(
    startNode: Node,
    ...
    if (startNode.parentNode)
      this.setStylesFromNodeHeirarchy(
  ...
  public setClassesFromNodeHiearchy(
    startNode: Node,
    ultimateParentNode: Node,
    owner: StyleOwner
  ) {
    ...
    if (startNode.isEqualNode(ultimateParentNode)) return;
    if (startNode.parentNode) {
      this.setClassesFromNodeHiearchy(
        startNode.parentNode,
        ultimateParentNode,
        owner
      );
  ...
  private getTheStyles(owner: StyleOwner, styleFilter?: string): Style[] {
    ...
    this.styles
      .filter(
        nodeStyle => nodeStyle.owner === owner && nodeStyle.type === "style"
      )
      .forEach(nodeStyle => {
        if (styleFilter) {
  ...
  private getClasses(owner: StyleOwner, styleFilter?: string): string {
    ...
    this.styles
      .filter(
        nodeStyle => nodeStyle.owner === owner && nodeStyle.type === "class"
      )
      .forEach(nodeStyle => {
        if (styleFilter) {
          if (nodeStyle.style.style !== styleFilter) {
