<template>
  <div
    class="w-full h-32 border border-gray-100 z-50 shadow-lg m-0"
    v-if="show"
  >
    <div
      class="w-full h-12 flex flex-row justify-between"
    ...
    >
      <div class="sidebar-button-panel">
        ...
        <icon-toggle-button
          :thisIconButton="italicButton"
          @onChange="onItalicClick"
        ></icon-toggle-button>
        <icon-toggle-button
          :thisIconButton="underLineButton"
          @onChange="onUnderlineClick"
        ></icon-toggle-button>
        <indent-outdent
...
export default class TextEditor extends Vue {
  ...
  fontWeightButton: ButtonIconClassList = new ButtonFactory().createButton(
    "class-list",
    "font-weight"
  ) as ButtonIconClassList;
  italicButton: ButtonIconClassInterface = new ButtonFactory().createButton(
    "class",
    "italic-button"
  ) as ButtonIconClassInterface;
  underLineButton: ButtonIconClassInterface = new ButtonFactory().createButton(
    "class",
    "underline-button"
  ) as ButtonIconClassInterface;
  ...
  onCloseClick(): void {
    const textEditor: HTMLParagraphElement = this.$refs
      .texteditorcontent as HTMLParagraphElement;
    PageModule.updateEditedComponentData(textEditor.innerHTML);
    SidebarModule.updateShowTextModal(false);
  }

  reset() {
    const textEditor: HTMLDivElement = this.$refs
      .texteditorcontent as HTMLDivElement;
