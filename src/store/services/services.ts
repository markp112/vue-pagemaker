...
class ServicesStore extends VuexModule implements ServicesStateInterface {
  ...
  public firestoreSaveColourPalette(data: {
    ...
    if (siteAndUserId.validate()) {
      return new Promise((resolve, reject) => {
        ...
        this.context.commit(
          "setCollectionId",
          `${data.siteAndUserId.userId}${data.siteAndUserId.siteId}${this.SETTINGS}`
        );
        firestore
          .collection(this.collectionId)
          .doc(this.PALETTE)
          .set(data.colourPalette)
          .then(() => {
  ...
  @Action
  public firestoreLoadSitePalette(
    siteIdAndUserId: SiteIdAndUserId
  ...
  firestoreSavePage(
    ...
    return new Promise((resolve, reject) => {
      const collectionId = `${userId}${siteId}::${this.PAGES}`;
      const firestore = firebase.firestore();
      firestore
        .collection(collectionId)
        .doc(pageId)
        .set(pageData)
        .then(() => {
          notification.message = "Page content saved";
          notification.status = "ok";
          resolve(notification);
        })
        .catch(err => {
  ...
  public firestoreSaveSiteDefaults(siteDefaultsPlusSiteAnduserId: {
    ...
    if (!siteDefaultsPlusSiteAnduserId)
      throw new Error(`${ERROR_MESSAGE} missing data`);
    const siteId = siteDefaultsPlusSiteAnduserId.siteDefaults.siteId;
    if (!siteId)
      throw new Error(`${ERROR_MESSAGE} siteId missing cannot be blank`);
    const userId = siteDefaultsPlusSiteAnduserId.siteDefaults.userId;
    return new Promise((resolve, reject) => {
  ...
  @Action({ rawError: true })
  public firestoreGetSiteDefaultSettings(data: {
    userId: string;
    siteId: string;
  }): Promise<SiteDefaultsInterface | Notification> {
    let siteDefaults: SiteDefaultsInterface;
    return new Promise((resolve, reject) => {
      this.context.commit(
        "setCollectionId",
        `${data.userId}${data.siteId}${this.SETTINGS}`
      );
      const firestore = firebase.firestore();
      firestore
        .collection(this.collectionId)
