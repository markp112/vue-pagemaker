    ...
    <div class="flex flex-row justify-start h-full overflow-hidden">
      ...
      <div class="flex flex-col justify-start ml-5 mt-5 w-9/12">
        ...
        <div class="relative bg-gray-300">
          <div class="mt-2 absolute" :class="classDef" :style="getStyles">
            Component
          </div>
        </div>
      </div>
...
export default class SidebarIconEditor extends Vue {
  ...
  iconClicked(icon: IconInterface) {
    const component: ComponentDefinitionInterface = SidebarModule.getSidebarAllIcons.filter(
      element => element.sidebarIcon.icon === icon.icon
    )[0];
    ...
    this.editorComponent.boxDimensions = new BoxDimensions(component.boxDimensions);
    this.iconLocal = icon;
  ...
  saveClick(): void {
    ...
    const component: ComponentDefinitionInterface = this.editorComponent
      .toObject as ComponentDefinitionInterface;
    SidebarModule.saveEditorElement(component).then(result => {
